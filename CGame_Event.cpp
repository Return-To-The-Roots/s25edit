...
void CGame::EventHandling(SDL_Event* Event)
{
    switch(Event->type)
    {
        ...
        case SDL_WINDOWEVENT:
        {
            ...
            {
                // In fullscreen the compositor (e.g. Wayland) may report the
                // native desktop size even though we render at GameResolution.
                // Keep the logical resolution unchanged and scale the output.
                if(fullscreen)
                    break;
                const Extent newSize(Event->window.data1, Event->window.data2);
                // Ignore stale resize events generated by our own window recreation.
                if(newSize == lastSetResolution_)
                    break;
