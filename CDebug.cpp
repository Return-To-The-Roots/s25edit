...
{
    dbgWnd = new CWindow(dbgCallback, quitParam, 0, 0, 540, 140, "Debugger", WINDOW_GREEN1, WINDOW_CLOSE | WINDOW_MOVE | WINDOW_MINIMIZE | WINDOW_RESIZE);
...
void CDebug::sendParam(int Param)
{
    switch (Param)
    {
        ...
        case CHANGE_SHADING:        if (CSurface::gouraud)
                                    ...
                                    if (global::s2->Map != NULL)
                                        global::s2->Map->needSurface = true;
...
void CDebug::actualizeData(void)
{
    ...
    if (global::s2->Map != NULL)
    {
        ...
        sprintf(puffer1, "Map Name: %s", global::s2->Map->map->name);
        MapNameText = dbgWnd->addText(puffer1, 260, 10, fontsize);
        if (MapSizeText != NULL)
            dbgWnd->delText(MapSizeText);
        sprintf(puffer1, "Width: %d  Height: %d", global::s2->Map->map->width, global::s2->Map->map->height);
        MapSizeText = dbgWnd->addText(puffer1, 260, 20, fontsize);
        if (MapAuthorText != NULL)
            dbgWnd->delText(MapAuthorText);
        sprintf(puffer1, "Author: %s", global::s2->Map->map->author);
        MapAuthorText = dbgWnd->addText(puffer1, 260, 30, fontsize);
        if (MapTypeText != NULL)
            dbgWnd->delText(MapTypeText);
        sprintf(puffer1, "Type: %d (%s)", global::s2->Map->map->type, (global::s2->Map->map->type == MAP_GREENLAND ? "Greenland" : (global::s2->Map->map->type == MAP_WASTELAND ? "Wasteland" : (global::s2->Map->map->type == MAP_WINTERLAND ? "Winterland" : "Unknown"))));
        MapTypeText = dbgWnd->addText(puffer1, 260, 40, fontsize);
        if (MapPlayerText != NULL)
            dbgWnd->delText(MapPlayerText);
        sprintf(puffer1, "Player: %d", global::s2->Map->map->player);
        MapPlayerText = dbgWnd->addText(puffer1, 260, 50, fontsize);
        if (VertexText != NULL)
            dbgWnd->delText(VertexText);
        sprintf(puffer1, "Vertex: %d, %d", global::s2->Map->VertexX, global::s2->Map->VertexY);
        VertexText = dbgWnd->addText(puffer1, 260, 60, fontsize);
        if (VertexDataText != NULL)
            dbgWnd->delText(VertexDataText);
        sprintf(puffer1, "Vertex Data: x=%ld, y=%ld, z=%d i=%.2f", global::s2->Map->map->vertex[global::s2->Map->VertexY*global::s2->Map->map->width+global::s2->Map->VertexX].x, global::s2->Map->map->vertex[global::s2->Map->VertexY*global::s2->Map->map->width+global::s2->Map->VertexX].y, global::s2->Map->map->vertex[global::s2->Map->VertexY*global::s2->Map->map->width+global::s2->Map->VertexX].z, ((float)global::s2->Map->map->vertex[global::s2->Map->VertexY*global::s2->Map->map->width+global::s2->Map->VertexX].i)/pow(2,16));
        VertexDataText = dbgWnd->addText(puffer1, 260, 70, fontsize);
        if (VertexVectorText != NULL)
            dbgWnd->delText(VertexVectorText);
        sprintf(puffer1, "Vertex Vector: (%.2f, %.2f, %.2f)", global::s2->Map->map->vertex[global::s2->Map->VertexY*global::s2->Map->map->width+global::s2->Map->VertexX].normVector.x, global::s2->Map->map->vertex[global::s2->Map->VertexY*global::s2->Map->map->width+global::s2->Map->VertexX].normVector.y, global::s2->Map->map->vertex[global::s2->Map->VertexY*global::s2->Map->map->width+global::s2->Map->VertexX].normVector.z);
        VertexVectorText = dbgWnd->addText(puffer1, 260, 80, fontsize);
        if (FlatVectorText != NULL)
            dbgWnd->delText(FlatVectorText);
        sprintf(puffer1, "Flat Vector: (%.2f, %.2f, %.2f)", global::s2->Map->map->vertex[global::s2->Map->VertexY*global::s2->Map->map->width+global::s2->Map->VertexX].flatVector.x, global::s2->Map->map->vertex[global::s2->Map->VertexY*global::s2->Map->map->width+global::s2->Map->VertexX].flatVector.y, global::s2->Map->map->vertex[global::s2->Map->VertexY*global::s2->Map->map->width+global::s2->Map->VertexX].flatVector.z);
